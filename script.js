...
fetch(apiNEWS)
	...
	.then(data => {
		...
		for (i = 0; i < dailyNews.length; i++) {
			...
			document.getElementById("title" + (i + 1)).innerText = dailyNews[i][0];
		}
...
function readOutLoud(message) {
	...
	//Responses about creator, jokes, thank you
		...
		message.search("tell me something about you") >= 0 ||
		message.search("who are you named after") >= 0
	) {
		botText +=
			"Hey everyone! My name is Alan. I am named after Alan Mathison Turing.";
		botText +=
			" Yes, Math is my middle name. Alan Turing is considered the father of theoretical Computer Science and Artificial Intelligence";
	} else if (
	...
	//Responses about date, day or time
		...
		message.search("what is the date today") >= 0 ||
		message.search("what is the date") >= 0 ||
		message.search("give me the date") >= 0 ||
		message.search("what is today's date") >= 0 ||
...
function scrollDiv_init() {
	DivElmnt = document.getElementById("scroll-news");
		...
		navigator.geolocation.getCurrentPosition(position => {
			...
			fetch(api)
				...
				.then(data => {
					...
					document.getElementById("1").textContent = temp + "°F";
					document.getElementById("2").textContent = humid * 100 + "%";
					document.getElementById("3").textContent = windspeed + " Mph";
					document.getElementById("4").textContent = uvindex;
					document.getElementById("myHeader").textContent = currentSummary;
				});
		});
	}
};
